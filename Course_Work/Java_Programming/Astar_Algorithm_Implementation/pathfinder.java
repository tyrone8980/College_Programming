...
public class pathfinder extends JFrame
{
    ...
    public static final boolean showtrace = true; // does not erase image
...
try {
	...
	diamondgif = ImageIO.read(new File(fpath+"poke.png"));
	prepareImage(diamondgif,this);
	mangif = ImageIO.read(new File(fpath+"pika.png"));
	...
	imagechar[astar.WATER] =
	    ImageIO.read(new File(fpath+"pika.png"));
 ...
 // my bridge
  BufferedImage imb = ImageIO.read(new File(fpath+"bridge1.png"));
  iw = imb.getWidth(null); ih = imb.getHeight(null);
  imageof[astar.DESERT] = new BufferedImage(gap*2,gap*2,BufferedImage.TYPE_INT_RGB);
  Gg = imageof[astar.DESERT].getGraphics();
  //	Gg.setClip(Hi);
  Gg.drawImage(imb,0,0,null);
